package contest.week135;


import model.TreeNode;
import util.MyCollectionUtil;


/**
 * 
 * Given the root of a binary search tree with distinct values, modify it so that every node has a new value equal to the sum of the values of the original tree that are greater than or equal to node.val.
As a reminder, a binary search tree is a tree that satisfies these constraints:
The left subtree of a node contains only nodes with keys less than the node's key.
The right subtree of a node contains only nodes with keys greater than the node's key.
Both the left and right subtrees must also be binary search trees.
 * 
 * The number of nodes in the tree is between 1 and 100.
Each node will have value between 0 and 100.
The given tree is a binary search tree.
 * 
 * 
 * @author angilin
 *
 */
public class P1038BinarySearchTreeToGreaterSumTree {
	int sum = 0;
	
	public TreeNode bstToGst(TreeNode root) {
		walk(root);
		return root;
    }
	
	public void walk(TreeNode treeNode){
		if(treeNode!=null) {
			walk(treeNode.right);
			//System.out.println(treeNode.val);
			sum = sum + treeNode.val;
			treeNode.val = sum;
			//System.out.println(treeNode.val);
			walk(treeNode.left);
		}
	}

	
	public static void main(String[] args) {
		Integer[] array = {4,1,6,0,2,5,7,null,null,null,3,null,null,null,8};
		TreeNode root = MyCollectionUtil.createBinaryTreeByArray(array, 0);
		TreeNode result = new P1038BinarySearchTreeToGreaterSumTree().bstToGst(root);
		new P1038BinarySearchTreeToGreaterSumTree().print(result);
	}
	
	public void print(TreeNode treeNode){
		if(treeNode!=null) { 
			print(treeNode.right); 
			System.out.println(treeNode.val);
			print(treeNode.left);
		 }
	}
}
